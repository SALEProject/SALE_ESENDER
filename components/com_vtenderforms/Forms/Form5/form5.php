<?php
	...
	$formGenerator = new FormGenerator("F5", $lang);
	$xml = $formGenerator->createForm();
	
	$form = file_get_contents("components" . DS . "com_vtenderforms" . DS . "Forms" . DS . "Form5" . DS . "form" . $lang . ".html");
	$idSave = isset($_GET['ID'])?$_GET['ID']:-1;
	
	echo "<form target='_blank' action='index.php?option=com_vtenderforms&form=form5&action=xml' method='POST' id='frm'>
			<input type='hidden' name='nameForm' value='F5' />
