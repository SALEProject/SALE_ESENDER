<?php
	...
	$formGenerator = new FormGenerator("F16", $lang);
	$xml = $formGenerator->createForm();
	
	$form = file_get_contents("components" . DS . "com_vtenderforms" . DS . "Forms" . DS . "Form16" . DS . "form" . $lang . ".html");
	$idSave = isset($_GET['ID'])?$_GET['ID']:-1;
	
	echo "<form target='_blank' action='index.php?option=com_vtenderforms&form=form16&action=xml' method='POST' id='frm'>
			<input type='hidden' name='nameForm' value='F16' />
